...
Ejs.AddressManager.prototype = {
    ...
    regListItemEvent: function () {
        ...
        list.delegate('.modify', 'click', function (E) {
            ...
            parent.newAddressBtn.removeClass('open');
            if (form.length > 0) {
        ...
        list.delegate('.delete', 'click', function (E) {
            ...
            Ejs.dialogConfirm(E, '您确定要删除这条收货地址吗？', function(){
        ...
        // 取消添加地址
        parent.newAddress.delegate('.btn-cancel', 'click', function () {
            parent.newAddress.hide();
            parent.newAddressBtn.removeClass('open');
        });
    ...
    setDefaultAddress: function (item, id) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    item.parent().find(".single").removeClass("cur");
                    item.parent().find('.default').remove();
                    item.find(".single").addClass("cur");
                    item.find('.modify').after('<a class="default">默认收货地址</a>');
            ...
            success: function (data) {
                if (data.success) {
                    if (type === 'update') {
                        ...
                        parent.hideModify(form);
                        if (eval('(' + data.data.address + ')').defaultAddress) {
                            parent.setDefaultAddress(listItem, listItem.attr("data-id"));
                        }
                    } else if (type === 'add') {
    ...
    addToList: function (form, type, formData, listItem) {
        ...
        if (parent.page === 'order') {
            html += '<!--<a href="javascript:void(0);" class="delete">删除</a>-->' +
                ...
                '<span class="col1"><i></i>' + list.name + '</span>\n' +
                '<span class="col2">' + list.province.replace(/\,/g, "") + list.location +
                '<span class="phone">' + list.mobile + '</span></span>';
        } else {
            html += "<span class=\"col1\">" + list.name + "</span>"
                    + "<span class=\"col2\">" + list.mobile + "</span>"
                    + "<span class=\"col3\">" + list.province.replace(/\,/g, "") + list.location + "</span>"
                    + "<span class=\"col4 postcode\">" + list.zipCode + "</span>"
        ...
        if (type === 'update') {
            ...
            html = '<li class="row clearfix" data-id="' + addressId + '"><p class="single clearfix">' + html + '</p></li>';
