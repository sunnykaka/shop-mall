...
/**
 * 处理一些io接口的
 * Created by Canal.wen on 2014/6/27 14:42.
 */
public class IO {
    /**
     * classpath下的utf-8的文件内容读取成字符串
     * @param classpathUrl 按这样的形式写: classpath:json/employeeBaseFullInfo_update.json
     *                     或者 json/employeeBaseFullInfo_update.json (注意不能用 / 开头)
     * @return
     * @throws java.io.IOException
     */
    public static String classpathResource2String(String classpathUrl) throws IOException {
        if (!classpathUrl.startsWith(ResourceUtils.CLASSPATH_URL_PREFIX)){
            classpathUrl = ResourceUtils.CLASSPATH_URL_PREFIX + classpathUrl;
        }

        return Files.toString(ResourceUtils.getFile(classpathUrl), Charsets.UTF_8).trim();
