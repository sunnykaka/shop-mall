...
/**
 * 来自于hibernate的sql格式化. 注意在大sql时, 会有性能问题. 所以不建议格式化超过1k的sql
 * Created by Canal.wen on 2014/7/15 13:48.
 */
public class SqlFormat {
    ...
    static
    {
        ...
        MISC.add("select");
        MISC.add("on"); }
    private static class FormatProcess { boolean beginLine = true;
        ...
        public FormatProcess(String sql) { this.tokens = new StringTokenizer(sql, "()+*/-=<>'`\"[], \n\r\f\t", true);
        }
        ...
        private static boolean isFunctionName(String tok) {
            ...
            return (isIdentifier) && (!SqlFormat.LOGICAL.contains(tok)) && (!SqlFormat.END_CLAUSES.contains(tok)) && (!SqlFormat.QUANTIFIERS.contains(tok)) && (!SqlFormat.DML.contains(tok)) && (!SqlFormat.MISC.contains(tok));
        }
