...
public class PictureToSmall {
    ...
    public static BufferedImage resizeImage(BufferedImage image, int width, int height) {
        ...
        BufferedImage resizeImage = new BufferedImage(width, height, image.getType()==0?BufferedImage.TYPE_3BYTE_BGR:image.getType());
        BufferedImage filterImage = null;

        if (xFactor * yFactor > WORK_LOAD_FACTOR) {
            filterImage = new BufferedImage(width, image.getHeight(), image.getType()==0?BufferedImage.TYPE_3BYTE_BGR:image.getType());
            horizontalFilter(image, filterImage, xFactor);
            verticalFilter(filterImage, resizeImage, yFactor);
        } else {
            filterImage = new BufferedImage(image.getWidth(), height, image.getType()==0?BufferedImage.TYPE_3BYTE_BGR:image.getType());
    ...
    public static void main(String[] args) throws Exception {
        BufferedImage image = ImageIO.read(new File("D:/log/0.jpg"));
        ImageIO.write(resizeImage(image, 0.27, 0.17), "PNG", new File("D:/log/01.jpg"));
    }
}
