...
public class ExportExcelUtil {
     ...
     */
    public static void exportExcel(Map<String, List> dataMap, String[] headerList, String[] fieldNameList, OutputStream out) {
        if (dataMap == null)
        ...
        try {
            ...
            for (Map.Entry<String, List> entry : dataMap.entrySet()) {
                ...
                for (Object obj : sheetList) {
                    ...
                    // 一个订单有多个订单项时, 需要合并单元格
                    String itemSize = getValue("itemSize", obj);
                    int region = NumberUtils.toInt(itemSize);

                    cellIndex = 0;
                    for (String methodName : fieldNameList) {
                        // 列数据
                        cell = row.createCell(cellIndex);
                        String value = getValue(methodName, obj);
                        // 如果此列的数据为空, 则与上面几行(主要在于订单有几个订单项)的当前列合并单元格, 合并后会默认使用左上单元格的数据
                        if (StringUtils.isBlank(value) && region > 1) {
                            // 参数: 起始行号, 结束行号, 起始列号, 结束列号
                            sheet.addMergedRegion(new CellRangeAddress(rowIndex + 1 - region, rowIndex, cellIndex, cellIndex));
                        }
