...
public class RoleCheckFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        Account account = SessionUtils.getLoginAccount(httpServletRequest.getSession());
        ...
        if (currentRole == null) {
            ...
            if (roleIdList.size() == 0) {
                ...
                response.getWriter().print("No role");
                return;
            }
            //角色不止一个要定向到角色选择页面
            if (roleIdList.size() > 1) {
                ...
                String roleSelectUrl = urlBrokerFactory.getUrl("SelectRole").toString();
                httpServletResponse.sendRedirect(roleSelectUrl);
                return;
            }
        ...
        if (currentRole.hasPermission(requestURL)) {
            ...
            httpServletResponse.getWriter().print("{success:false,msg:'sorry you don't have permission'}");
        }
    }
